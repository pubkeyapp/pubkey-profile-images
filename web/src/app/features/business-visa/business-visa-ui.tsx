...
import { BusinessVisaPayload } from './business-visa-data-access'

export function BusinessVisaUiLayout({
  profiles,
  url,
  setParams,
}: {
  profiles: BusinessVisaPayload[]
  url: string
  setParams: (params: string) => void
}) {
  const [profile, setProfile] = useState<BusinessVisaPayload | undefined>(profiles[0])
  return (
      ...
      <Paper withBorder radius="md" shadow="lg">
        <Grid h="100%" gutter={0}>
          <Grid.Col span={8} p="md">
            <BusinessVisaUiImagePreview url={url} />
          </Grid.Col>
          <Grid.Col span={4} py="md" pr="md">
            <UiStack>
              <BusinessVisaUiForm profile={profile} setParams={setParams} />
...
export function BusinessVisaUiForm({
  profile,
  setParams,
}: {
  profile?: BusinessVisaPayload
  setParams: (params: string) => void
}) {
  function submit(profile?: BusinessVisaPayload) {
  ...
  const form = useForm<BusinessVisaPayload>({
    initialValues: {
      status: '',
      earnings: '',
      name: '',
...
export function BusinessVisaUiImagePreview({ url }: { url: string }) {
  return (
